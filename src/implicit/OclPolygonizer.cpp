...
namespace SKETCH {
	...
	GLMeshBuffer* GPUPoly::prepareMeshBufferForDrawingNormals(float len) {
		if(!m_gmbVertex.isValid() || !m_gmbNormal.isValid())
			return NULL;
	...
	int GPUPoly::init()
	{
		...
		LogInfo("1.Setup compute device. Prefer GPU.");
	...
	int GPUPoly::runTandem(float cellsize)
	{
		...
		*/		
		...
		U32 stepVertex = 4;
		U32 stepColor = 4;
		GLsizeiptr szVertexBuffer = ctSumVertices * stepVertex * sizeof(float);
		GLsizeiptr szNormalBuffer = ctSumVertices * 3 * sizeof(float);

		//Vertex
		m_gmbVertex.setup(gbtPosition, 4, GL_FLOAT, szVertexBuffer, NULL, gbuDynamicDraw);
		cl_mem outMemMeshVertex = m_lpGPU->createMemBufferFromGL(m_gmbVertex.handle(), PS::CL::memWriteOnly);

		//Color
		m_gmbColor.setup(gbtColor, 4, GL_FLOAT, szVertexBuffer, NULL, gbuDynamicDraw);
		cl_mem outMemMeshColor = m_lpGPU->createMemBufferFromGL(m_gmbColor.handle(), PS::CL::memWriteOnly);

		//Normal
		m_gmbNormal.setup(gbtNormal, 3, GL_FLOAT, szNormalBuffer, NULL, gbuDynamicDraw);
		cl_mem outMemMeshNormal = m_lpGPU->createMemBufferFromGL(m_gmbNormal.handle(), PS::CL::memWriteOnly);
	...
	bool GPUPoly::readbackMeshV3T3(U32& ctVertices, vector<float>& vertices,
		...
		//Vertices
		cl_mem outMemMeshVertices = m_lpGPU->createMemBufferFromGL(m_gmbVertex.handle(), PS::CL::memReadOnly);
		...
		//Elements
		cl_mem outMemMeshElements = m_lpGPU->createMemBufferFromGL(m_gmbFaces.handle(), PS::CL::memReadOnly);
	...
	bool GPUPoly::readBackNormals(U32& ctVertices, vector<float>& verticesXYZ, vector<float>& normals) const {
		...
		//Vertices
		cl_mem outMemMeshVertices = m_lpGPU->createMemBufferFromGL(m_gmbVertex.handle(), PS::CL::memReadOnly);
		cl_mem outMemMeshNormals = m_lpGPU->createMemBufferFromGL(m_gmbNormal.handle(), PS::CL::memReadOnly);
...
//													  __global float4* arrOutOffSurfaceXYZF)
		...
		outOffSurfacePoints.resize(m_gmbVertex.step() * ctOutVertices);
		U32 szOffSurfaceBuffer = sizeof(float) * m_gmbVertex.step() * ctOutVertices;
	...
	int GPUPoly::computeElements(U32 ctElements)
	{
		...
		//Elements
		m_gmbFaces.setup(gbtFaceIndex, 3, GL_UNSIGNED_INT, sizeof(U32) * ctElements, NULL, gbuDynamicDraw);
		cl_mem outMemMeshElements = m_lpGPU->createMemBufferFromGL(m_gmbFaces.handle(), PS::CL::memWriteOnly);
	...
	int GPUPoly::computeVertexAttribs(U32 ctVertices)
	{
		...
		U32 stepVertex = 4;
		U32 stepColor = 4;
		GLsizeiptr szVertexBuffer = ctVertices * stepVertex * sizeof(float);
		...
		//Vertex
		m_gmbVertex.setup(gbtPosition, stepVertex, GL_FLOAT, szVertexBuffer, NULL, gbuDynamicDraw);
		cl_mem outMemMeshVertex = m_lpGPU->createMemBufferFromGL(m_gmbVertex.handle(), PS::CL::memWriteOnly);

		//Color
		m_gmbColor.setup(gbtColor, stepColor, GL_FLOAT, szVertexBuffer, NULL, gbuDynamicDraw);
		cl_mem outMemMeshColor = m_lpGPU->createMemBufferFromGL(m_gmbColor.handle(), PS::CL::memWriteOnly);
