...
namespace PS {
    namespace SKETCH {
        ...
        int FieldComputer::fieldsForVoxelGrid(float cellsize, bool stackless) {
			...
			m_gmbVertex.setup(gbtPosition, 4, GL_FLOAT, sizeof(float) * 4 * grid.total, NULL, gbuDynamicDraw);

			cl_mem inoutMemVertices;
			inoutMemVertices = m_lpGPU->createMemBufferFromGL(m_gmbVertex.handle(), PS::CL::memReadWrite);
